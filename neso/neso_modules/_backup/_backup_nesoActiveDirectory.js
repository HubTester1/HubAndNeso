					...
					.on("end_parsed",function(adUsersRaw){
						...
						for (const adUserRaw of adUsersRaw) {
							...
							if ( typeof(adUserRaw.userPrincipalName) !== "undefined") {
								...
								var userAccount = nesoUtilities.StrInStr({ incomingHaystack: adUserRaw.userPrincipalName.toLowerCase(), incomingNeedle: "@mos.org", flag: 1});
								// var managerArray = adUserRaw.manager.split(",");
								// var managerAccount = nesoUtilities.StrInStr({ incomingHaystack: managerArray[0].toLowerCase(), incomingNeedle: "cn=", flag: 3});
								...
								for (const oneGroupWithAllDataString of allGroupsAllDataArray) {
									...
									if (nesoUtilities.StrInStr({ incomingHaystack: oneGroupWithAllDataArray[1], incomingNeedle: "Exchange"}) === false) {
										securityGroups.push(nesoUtilities.StrInStr({ incomingHaystack: oneGroupWithAllDataArray[0], incomingNeedle: "CN=", flag: 3}));
									}
								}

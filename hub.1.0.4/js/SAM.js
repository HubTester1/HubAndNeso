...
(function ($) {
	...
	$.fn.UserNeeedsAlternateOverviewScreen = function () {
		...
		console.log("mData.requestName");
		console.log(mData.requestName);

		console.log("userNeeedsAlternateOverviewScreen");
		console.log(userNeeedsAlternateOverviewScreen);
	...
	$.fn.GetJobValues = function (jobId) {
		...
		var opt = $.extend({}, {
			listName: "SWFList",
			webURL: "https://bmos.sharepoint.com/sites/" + hrOrHRP + "-service-jobs",
			completefunc: null
		}, options);
		...
		if (rData.requestID != "") {
			...
			$('#submit-signup').click(function () {
				...
				$().SPServices({
					...
					listName: "SWFList",
					webURL: "https://bmos.sharepoint.com/sites/" + hrOrHRP + "-service-signup",
					batchCmd: 'New',
					ID: 0,
	...
	$.fn.ConfigureRequest = function (passedRequestID) {
		...
		if (mData.requestName === "GSE Schedule" && uData.roles.indexOf("gseUserOnly") > -1) {
			...
			$("div.mos-drag-and-drop-file-attachment").on('drag dragstart dragend dragover dragenter dragleave drop', function (e) {
				e.preventDefault();
				e.stopPropagation();
			})
				...
				.on('dragleave dragend drop', function () {
					...
					$(this).find("div.mos-drag-and-drop-file-input label").html("Drop a file here, or <span class=\"mos-file-selection-prompt\">tap or click here to select one</span>.");
				})
				.on('drop', function (e) {
			...
			$("div.mos-drag-and-drop-file-attachment div.mos-drag-and-drop-file-input input.mos-file-selector").on("change", function (e) {
	...
	$.fn.ValidateInRealTimeForPositiveGrantCostsInUSD = function (value, element) {
		if (!(/^((\$)?([0-9]{1,3})?(?:,?[0-9]{3})*(?:\.[0-9]{1,2})?)?$/.test(value))) {
			$().SetErrorMessage(element, 'Please use numeric characters, optionally using commas to separate groups of three numerals and a decimal to separate the integer portion from the fractional portion of the number. Avoid all other characters, including spaces. Convert to USD, if grant will be in another currency.');
			return 0;
		} else {
			$().RemoveErrorMessage(element);
			return 1;
	...
	$.fn.BuildMOSFileField = function (e) {
		...
		field += ' ">' +
			...
			'				<label for="file_' + e.hypehnatedName + '">Drop a file here, or <span class="mos-file-selection-prompt">tap or click here to select one</span>.</label>' +
			'			</div>' +
	...
	$(document).ready(function () {
		...
		$.when.apply($, allDataRetrievalAndSettingPromises).always(function () {
			...
			console.log('using dev_mos-main_long.1.04 m1 - DevCodeSam');

(function($) {
        ...
        if ($("div#mos-form").ValidateForm() != false) {
            ...
            if (rData.lastModMismatch == 0) {
                ...
                if (mData.autoTrackPendingAndApprovalAndCompleted == 1 && rData.endOfLife != 1) {
                    ...
                    if (rData.requestStatus == '') {
                        newReqStatus = 'Pending Approval';
                    } else if ($("input#change-types_name").length == 1 && $("input#change-types_name:checked").length == 0) {
                        newReqStatus = 'Approved';
                    } else if (rData.requestStatus == 'Pending Approval' && $('select#Change-Request-Status option:selected').val() == 'Approve') {
                        newReqStatus = 'Approved';
                    } else if (rData.requestStatus == 'Pending Approval' && ($('input#requester-cancellation_cancel:checked').length > 0 || $('select#Change-Request-Status option:selected').val() == 'Cancel')) {
    ...
    $.fn.BuildListItemChooserField = function(e, isAdmin, rStatus) {
        ...
        if (editable == 0) {
            ...
            if (e.fieldName == "Event Request ID") {
                field += '        <a href="" target="_blank" data-source-type="event-request"';
            }
        } else  {
        ...
        for (field in formDataCopy) {
            ...
            if (field === "RepeatedElements") {
                ...
                if (formDataCopy[field] != '') {
                    ...
                    if ($(element).attr('data-source-type') == 'event-request') {
                        $(element).attr('href', 'https://bmos.sharepoint.com/sites/vxo-function/SitePages/Event Request.aspx?requestID=' + formDataCopy[field]).text('#' + formDataCopy[field]);
                    }

                    if ($(element).attr('data-source-type') == 'event-space-request') {
                        $(element).attr('href', 'https://bmos.sharepoint.com/sites/vxo-function-space/SitePages/Event Space Request.aspx?requestID=' + formDataCopy[field]).text('#' + formDataCopy[field]);
                    }
    ...
    function BuildAllRequestDataObject(formElement) {
        ...
        $(formElement).find('a[data-source-type="url"], a[data-source-type="file"], a[data-source-type="event-request"]').each(function() {
            id = this.id;
    ...
    $.fn.ImportEventRequestDataToCateringRequest = function(requestID) {
